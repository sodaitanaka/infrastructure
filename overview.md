# T3 Recruit Automation  
## 開発見積 概要資料

**宛先：** T3株式会社 御中  
**発行：** 株式会社homula  
**発行日：** 2026年3月

---

## 1. システム概要

OfferBoxから候補者レジュメを自動取得し、AIによるパーソナライズメッセージを生成・品質検証のうえ自動送信する採用オファー自動化システムの開発見積です。

本資料では **n8nあり構成（Phase2推奨）** と **フルスクラッチ構成（Phase3候補）** の2案を提示します。

---

## 2. 処理フロー概要

```mermaid
flowchart LR
    A["ブックマーク登録\n（T3担当者）"] --> B["レジュメ取得\nRPA / RoboCorp"]
    B --> C["AIメッセージ生成\nClaude API"]
    C --> D["品質検証\nHG-01〜04 + Gemini スコア"]
    D --> E{承認フロー}
    E -->|Auto 80点以上| F["自動送信\nOfferBox"]
    E -->|Review 50〜79点| G["承認待ちキュー\n担当者が判定"]
    E -->|NG 49点以下| H["送信禁止\nログ記録"]
    G -->|承認| F
    F --> I["送信ログ記録\nPostgreSQL"]
```

---

## 3. 共通スタック

ユーザー画面・管理画面・DB・AI・RPA層は **両構成で共通**。変わるのはオーケストレーション層のみ。

### 3-1. フロントエンド

| カテゴリ | 技術 | 備考 |
|---------|------|------|
| フレームワーク | Next.js 14（App Router） | SSR対応。日本エンジニア採用市場で主流 |
| 言語 | TypeScript | 型安全性確保。バグ早期検出 |
| スタイリング | Tailwind CSS | デザイントークンと整合 |
| コンポーネント | shadcn/ui | アクセシビリティ対応済み |
| 状態管理 | React Query（TanStack Query） | サーバー状態のキャッシュ・再フェッチ管理 |
| フォーム | React Hook Form + Zod | 入力バリデーション |
| デプロイ | Vercel | Next.jsとの親和性が高い。Preview環境が自動生成 |

### 3-2. バックエンド

| カテゴリ | 技術 | 備考 |
|---------|------|------|
| APIフレームワーク | Next.js API Routes | フロントと同一リポジトリで完結 |
| 認証 | NextAuth.js v5（Auth.js） | メール+PW認証。JWT。ユーザー/管理者ロール分離 |
| ORM | Prisma | PostgreSQLとのDB操作。マイグレーション管理 |
| バリデーション | Zod | APIリクエストのスキーマ検証 |
| 暗号化 | Node.js crypto（AES-256-GCM） | APIキーの暗号化保存 |
| メール送信 | Resend | パスワードリセットメール送信 |

### 3-3. データベース

| カテゴリ | 技術 | 備考 |
|---------|------|------|
| DB本体 | PostgreSQL | テナント管理・ジョブログ・送信ログ・APIキー保管 |
| ホスティング | Supabase | RLS（Row Level Security）でマルチテナント分離 |
| マイグレーション | Prisma Migrate | スキーマ変更の履歴管理 |

### 3-4. AI層

| カテゴリ | 技術 | 備考 |
|---------|------|------|
| メッセージ生成LLM | Claude API（claude-sonnet-4-5） | レジュメ解析・候補者分類・メッセージ生成（統合処理） |
| 品質スコアリングLLM | Gemini API（Gemini 2.0 Flash） | 生成メッセージの安全性を0〜100点で評価 |
| コスト最適化 | Prompt Caching（Claude） | システムプロンプトをキャッシュし入力トークンコストを削減 |

### 3-5. RPA層

| カテゴリ | 技術 | 備考 |
|---------|------|------|
| RPAフレームワーク | RoboCorp Cloud | OfferBoxへのログイン・レジュメ取得・メッセージ送信 |
| ブラウザ操作 | Selenium 4 / Playwright | 採用媒体のUI操作 |
| BAN対策 | プロキシローテーション | IPアドレス切り替えによるアクセス制限回避 |
| 待機制御 | ランダム待機（3〜7秒） | 送信間隔の揺らぎで不自然なアクセスを回避 |

### 3-6. インフラ・運用共通

| カテゴリ | 技術 | 備考 |
|---------|------|------|
| エラー通知 | Slack Webhook | Failed連続3件・LLMエラー率超過時の即時通知 |
| ログ監視 | Vercel Logs / Supabase Logs | APIエラー・クエリパフォーマンス監視 |
| シークレット管理 | Vercel Environment Variables | APIキー・DB接続文字列の安全な管理 |
| CI/CD | GitHub Actions + Vercel | PRマージで自動デプロイ |

---

## 4. 構成比較

```mermaid
flowchart TD
    A["採用オファー自動化システム"] --> B["🟢 n8nあり構成\nPhase2 推奨"]
    A --> C["🔵 フルスクラッチ構成\nPhase3 候補"]

    B --> B1["オーケストレーション\nn8n Cloud GUI"]
    B --> B2["スケジューラ\nn8n Cron Trigger"]
    B --> B3["テナント管理\nワークフロー自動コピー"]
    B --> B4["ランニングコスト\n+$20〜/月"]

    C --> C1["オーケストレーション\nPython / FastAPI Worker"]
    C --> C2["スケジューラ\npg-boss（PostgreSQL）"]
    C --> C3["テナント管理\nDBレコード追加のみ"]
    C --> C4["ランニングコスト\n+$0〜$10/月"]

    style B fill:#F0FDF4,stroke:#22C55E,stroke-width:2px
    style C fill:#EFF6FF,stroke:#3B82F6,stroke-width:2px
```

---

## 5. アーキテクチャ図

### 5-1. n8nあり構成

```mermaid
flowchart LR
    subgraph USER["👤 ユーザー層"]
        U1["採用担当者\n（クライアント企業）"]
        U2["T3管理者\n（admin）"]
    end

    subgraph FE["🖥️ フロントエンド / Next.js"]
        FE1["ユーザー画面 6画面\n①ログイン ②TOP\n③プロンプト設定\n④送信時間設定\n⑤ログ閲覧 ⑥会社設定"]
        FE2["管理画面 4画面\n①管理者ログイン\n②テナント一覧\n③テナント作成\n④ジョブ監視"]
    end

    subgraph BE["⚙️ バックエンド / Next.js API Routes"]
        BE1["認証\nNextAuth.js + JWT"]
        BE2["テナント管理 API"]
        BE3["n8n ゲートウェイ\nワークフロー操作ラッパー"]
        BE4["ログ集約 API"]
        BE5["APIキー管理\nAES-256暗号化"]
    end

    subgraph DB["🗄️ PostgreSQL / Supabase"]
        DB1["tenants"]
        DB2["job_logs"]
        DB3["api_keys"]
        DB4["send_logs"]
    end

    subgraph N8N["🔄 n8n Cloud"]
        N1["ワークフロー群\nテナント×媒体ごとにコピー"]
        N2["Cron Trigger"]
        N3["Webhook / RPA連携"]
    end

    subgraph AI["🤖 AI層"]
        AI1["Claude API\nレジュメ解析・分類・生成"]
        AI2["Gemini API\n品質スコアリング"]
    end

    subgraph RPA["🤖 RPA / RoboCorp Cloud"]
        R1["ブックマーク取得"]
        R2["レジュメスクレイピング"]
        R3["メッセージ自動送信"]
    end

    U1 --> FE1
    U2 --> FE2
    FE1 --> BE1 & BE3 & BE4
    FE2 --> BE2 & BE3 & BE4
    BE1 --> DB1
    BE2 --> DB1
    BE3 -->|n8n REST API| N1
    BE3 --> N2
    BE4 --> DB2 & DB4
    BE5 --> DB3
    N1 --> AI1 & AI2
    N2 -->|Cron起動| R1
    R1 --> R2
    R2 -->|レジュメ| N3 --> N1
    N1 -->|生成メッセージ| R3
    R3 -->|送信| M1["OfferBox"]
```

### 5-2. フルスクラッチ構成

```mermaid
flowchart LR
    subgraph USER["👤 ユーザー層"]
        U1["採用担当者\n（クライアント企業）"]
        U2["T3管理者\n（admin）"]
    end

    subgraph FE["🖥️ フロントエンド / Next.js"]
        FE1["ユーザー画面 6画面"]
        FE2["管理画面 4画面"]
    end

    subgraph BE["⚙️ バックエンド / Next.js API Routes"]
        BE1["認証\nNextAuth.js + JWT"]
        BE2["テナント管理 API"]
        BE4["ログ集約 API"]
        BE5["APIキー管理\nAES-256暗号化"]
    end

    subgraph QUEUE["⏱️ pg-boss / PostgreSQL"]
        Q1["Cron スケジューラ"]
        Q2["ジョブキュー"]
        Q3["リトライ制御"]
    end

    subgraph ORCH["🔀 FastAPI Worker / Python"]
        O1["レジュメ検証"]
        O2["時期判定\nearly/mid/late"]
        O3["プロンプト構築"]
        O4["HG-01〜04 検証"]
        O5["承認フロー制御\nAuto / Review / NG"]
        O6["二重送信チェック"]
    end

    subgraph DB["🗄️ PostgreSQL / Supabase"]
        DB1["tenants"]
        DB2["job_logs"]
        DB3["api_keys"]
        DB4["send_logs"]
        DB5["prompts"]
        DB6["approval_queue"]
    end

    subgraph AI["🤖 AI層"]
        AI1["Claude API\nレジュメ解析・分類・生成"]
        AI2["Gemini API\n品質スコアリング"]
    end

    subgraph RPA["🤖 RPA / RoboCorp Cloud"]
        R1["ブックマーク取得"]
        R2["レジュメスクレイピング"]
        R3["メッセージ自動送信"]
    end

    U1 --> FE1
    U2 --> FE2
    FE1 --> BE1 & BE4
    FE2 --> BE2 & BE4
    BE1 --> DB1
    BE2 -->|ジョブ登録| Q1
    BE4 --> DB2
    BE5 --> DB3
    Q1 --> Q2 --> R1 & ORCH
    Q3 -.->|リトライ| Q2
    R1 --> R2
    R2 --> O1 --> O2 --> O3
    O3 --> AI1
    AI1 --> O4 --> AI2 --> O5
    O5 -->|Auto| O6 --> R3
    O5 -->|Review| DB6
    O5 -->|NG| DB4
    DB5 --> O3
    DB6 -->|承認操作| FE1
    R3 -->|送信| M1["OfferBox"]
    R3 --> DB4
```

---

## 6. オーケストレーション層の比較

| 観点 | n8nあり ✅ | フルスクラッチ |
|------|-----------|--------------|
| 開発工数 | ベースライン | +29〜30人日 |
| テナント追加 | ワークフローコピー（API自動化で解決可能） | DBレコード追加のみ（ゼロ工数） |
| 処理ロジックの可視性 | GUIで視覚的に確認可能 | コードで完結。テスト・レビューが容易 |
| デバッグ | n8n実行ログで確認 | 通常のログ・トレースで確認 |
| ベンダーロック | 高い（n8n固有JSON構造に依存） | なし（標準Python/SQLのみ） |
| スケーラビリティ | テナント100社超でワークフロー量産が課題 | DBスケールのみで対応 |
| 月額コスト差分 | +$20〜/月（n8n Cloud） | +$0〜$10/月（Workerホスティングのみ） |
| 推奨タイミング | 現時点（Phase1〜2初期） | テナント50社超 or n8n API制限発覚時 |

### フルスクラッチへの移行トリガー

```mermaid
flowchart TD
    T["移行検討開始"] --> T1["① テナント50社超\nワークフロー量産の運用負荷が顕在化"]
    T --> T2["② n8n Cloud API制限発覚\nプロンプト書き換え・スケジュール変更に制限\n（Week1スパイクで確認）"]
    T --> T3["③ 高度AI機能が必要になった\nRAG・会話履歴・ファインチューニング等"]
    T --> T4["④ n8n Cloud月額が$200超\nテナント数増加によるコスト超過"]

    style T fill:#FEF3C7,stroke:#F59E0B,stroke-width:2px
```

---

## 7. 単価前提

| 役割 | 時間単価（税抜） | 対象タスク |
|------|---------------|-----------|
| PM | 12,500円/h | スケジュール管理・リスク管理・社内調整・進捗報告 |
| ディレクション | 12,500円/h | 定例会議・議事録・T3様窓口・仕様調整 |
| コンサル | 15,000円/h | 要件定義・設計・テスト計画書作成・スパイク |
| エンジニア | 12,500円/h | 実装・結合確認・内部テスト・バグ修正 |
| アシスタント | 5,000円/h | テストケース作成補助・UAT補助・マニュアル作成 |

> ※ 1人日 = 8時間  
> ※ 別途消費税10%が加算されます

---

## 8. 費用見積

### 8-1. n8nあり構成（Phase2）

**体制：** 2名（フルスタックエンジニア + フロントエンドエンジニア）  
**期間：** 8〜10週間（2026/04/01 〜 2026/06/13）

| 役割 | 時間単価 | 工数（人日） | 工数（時間） | 費用（税抜） |
|------|---------|------------|------------|------------|
| PM | 12,500円 | 5人日 | 40h | 500,000円 |
| ディレクション | 12,500円 | 9人日 | 72h | 900,000円 |
| コンサル | 15,000円 | 12人日 | 96h | 1,440,000円 |
| エンジニア | 12,500円 | 89人日 | 712h | 8,900,000円 |
| アシスタント | 5,000円 | 6人日 | 48h | 240,000円 |
| **小計** | | **121人日** | **968h** | **11,980,000円** |
| 消費税（10%） | | | | 1,198,000円 |
| **合計（税込）** | | **121人日** | **968h** | **13,178,000円** |

**工数内訳**

| 役割 | 内訳 | 人日 |
|------|------|------|
| PM | 全体管理・進捗報告（10週 × 0.5人日/週） | 5人日 |
| ディレクション | 定例会議5日 ＋ T3様レビュー・UAT対応・納品調整4日 | 9人日 |
| コンサル | DB・API・認証設計9日 ＋ テスト計画書3日 | 12人日 |
| エンジニア | バックエンド25日 ＋ フロントエンド10画面38日 ＋ 結合確認・E2Eテスト12日 ＋ バグ修正・n8nスパイク14日 | 89人日 |
| アシスタント | テストケース作成補助・UAT補助・操作マニュアル作成 | 6人日 |

---

### 8-2. フルスクラッチ構成（Phase3）

**体制：** 3名（フルスタックエンジニア + Pythonエンジニア + フロントエンドエンジニア）  
**期間：** 15〜17週間（2026/04/01 〜 2026/07/13）

| 役割 | 時間単価 | 工数（人日） | 工数（時間） | 費用（税抜） |
|------|---------|------------|------------|------------|
| PM | 12,500円 | 7人日 | 56h | 700,000円 |
| ディレクション | 12,500円 | 14人日 | 112h | 1,400,000円 |
| コンサル | 15,000円 | 20人日 | 160h | 2,400,000円 |
| エンジニア | 12,500円 | 119人日 | 952h | 11,900,000円 |
| アシスタント | 5,000円 | 8人日 | 64h | 320,000円 |
| **小計** | | **168人日** | **1,344h** | **16,720,000円** |
| 消費税（10%） | | | | 1,672,000円 |
| **合計（税込）** | | **168人日** | **1,344h** | **18,392,000円** |

**工数内訳**

| 役割 | 内訳 | 人日 |
|------|------|------|
| PM | 全体管理・進捗報告（17週 × 0.4人日/週） | 7人日 |
| ディレクション | 定例会議8日 ＋ T3様レビュー・UAT対応・納品調整6日 | 14人日 |
| コンサル | DB・API・認証・Workerアーキテクチャ設計14日 ＋ テスト計画書6日 | 20人日 |
| エンジニア | バックエンド14日 ＋ FE30日 ＋ Worker21日 ＋ pg-boss8日 ＋ 冪等性7日 ＋ インフラ3日 ＋ 結合10日 ＋ 負荷テスト5日 ＋ テスト・修正21日 | 119人日 |
| アシスタント | テストケース作成補助・UAT補助・操作マニュアル作成 | 8人日 |

---

## 9. 比較サマリー

```mermaid
xychart-beta
    title "役割別費用比較（税抜・万円）"
    x-axis ["PM", "ディレクション", "コンサル", "エンジニア", "アシスタント"]
    y-axis "費用（万円）" 0 --> 1200
    bar [50, 90, 144, 890, 24]
    bar [70, 140, 240, 1190, 32]
```

| | n8nあり（Phase2） | フルスクラッチ（Phase3） |
|---|---|---|
| **合計工数** | 121人日 / 968h | 168人日 / 1,344h |
| **合計費用（税抜）** | **11,980,000円** | **16,720,000円** |
| **合計費用（税込）** | **13,178,000円** | **18,392,000円** |
| 費用差額（税抜） | ─ | +4,740,000円 |
| 開発期間 | 8〜10週間 | 15〜17週間（約1.7倍） |
| 必要エンジニア | 2名（FS + FE） | 3名（FS + PY + FE） |
| 納品予定日 | 2026/06/10 | 2026/07/10 |
| リリース予定日 | 2026/06/13 | 2026/07/13 |

---

## 10. スケジュール概要

### 10-1. n8nあり構成

```mermaid
gantt
    title n8nあり構成 開発スケジュール
    dateFormat YYYY-MM-DD
    axisFormat %m/%d

    section 設計
    DB・API・認証設計          :2026-04-01, 3d
    n8n APIスパイク             :2026-04-01, 4d
    開発環境構築                :2026-04-03, 5d
    T3様設計レビュー            :2026-04-07, 2d

    section 実装
    認証基盤・ログイン画面      :2026-04-10, 5d
    テナント管理・n8nゲートウェイ :2026-04-14, 11d
    TOP・プロンプト・送信時間   :2026-04-21, 12d
    ログ・ジョブ監視・APIキー   :2026-05-02, 12d

    section テスト・リリース
    結合確認                    :2026-05-14, 5d
    内部テスト                  :2026-05-19, 4d
    T3様UAT                     :2026-05-26, 5d
    修正                        :2026-05-30, 7d
    納品                        :milestone, 2026-06-10, 0d
    リリース                    :milestone, 2026-06-13, 0d
```

### 10-2. フルスクラッチ構成

```mermaid
gantt
    title フルスクラッチ構成 開発スケジュール
    dateFormat YYYY-MM-DD
    axisFormat %m/%d

    section 設計
    DB・API・認証・Worker設計   :2026-04-01, 4d
    開発環境構築                :2026-04-03, 5d
    T3様設計レビュー            :2026-04-09, 2d

    section 実装（FE/BE）
    認証基盤・ログイン画面      :2026-04-14, 5d
    テナント管理                :2026-04-21, 7d
    TOP・プロンプト・送信時間   :2026-05-02, 14d
    ログ・ジョブ監視・APIキー   :2026-05-14, 16d

    section 実装（Worker）
    FastAPI Worker・Webhook     :2026-04-14, 5d
    時期判定・Claude・HG検証    :2026-04-18, 12d
    Gemini・承認・dedup・DB     :2026-04-30, 10d
    pg-boss Cronスケジューラ    :2026-04-28, 10d
    冪等性・並列制御            :2026-05-14, 7d
    Workerインフラ・負荷テスト  :2026-05-21, 9d

    section テスト・リリース
    結合確認                    :2026-06-01, 5d
    内部テスト                  :2026-06-08, 5d
    T3様UAT                     :2026-06-16, 5d
    修正                        :2026-06-22, 11d
    納品                        :milestone, 2026-07-10, 0d
    リリース                    :milestone, 2026-07-13, 0d
```

---

*以上*  
*株式会社homula*
