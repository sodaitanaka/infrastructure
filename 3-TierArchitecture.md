...
・ビルの外 = インターネット（誰でもアクセスできる）
・ビルの中 = VPC（自分だけのプライベート空間）
・ビルの各フロア = サブネット（役割ごとに区切った部屋）
・フロアの入退室ルール = セキュリティグループ / ネットワークACL
・ビルの玄関 = インターネットゲートウェイ（外との唯一の出入口）
```
...
graph TD
    ...
    subgraph VPC["VPC 10.0.0.0/16（自分専用のビル）"]
        subgraph AZ_A["アベイラビリティゾーン A"]
            PubA["📦 パブリックサブネット A\n10.0.1.0/24\n（1Fフロント）"]
            PriA["🔒 プライベートサブネット A\n10.0.2.0/24\n（地下金庫）"]
        end
        subgraph AZ_B["アベイラビリティゾーン B（冗長化）"]
            PubB["📦 パブリックサブネット B\n10.0.3.0/24"]
            PriB["🔒 プライベートサブネット B\n10.0.4.0/24"]
        end
    end
...
**一言で：** AWS上に作る「自分専用の仮想ネットワーク空間」

| 項目 | 内容 |
|---|---|
| 何を決めるか | IPアドレスの範囲（例：10.0.0.0/16） |
| デフォルトの状態 | 外部とは完全に遮断されている |
| AWSアカウントとの関係 | 複数のVPCを作れる（本番・開発で分けるなど） |

**IPアドレス範囲の読み方（超基本）：**
```
10.0.0.0/16 → 10.0.○○○.○○○ の範囲が使える
　→ 約65,000個のIPアドレスが使える広大な空間

サブネットはこの中から切り出して使う
10.0.1.0/24 → 約254個（1フロア分）
10.0.2.0/24 → 約254個（別の1フロア分）
```

> SAAでは「/16より/24の方が狭い」という感覚だけ持てばOK

---

### ② サブネット（Subnet）

**一言で：** VPCの中を「役割ごとに区切った部屋」

#### パブリックサブネット vs プライベートサブネット

決定的な違いは**インターネットゲートウェイへのルートがあるかどうか**だけです。

| | パブリックサブネット | プライベートサブネット |
|---|---|---|
| インターネットから到達できるか | ✅ できる | 🚫 できない |
| ルートテーブルの設定 | IGWへのルートあり | IGWへのルートなし |
| 置くもの | Webサーバー、API、踏み台サーバー | DB（RDS）、バックエンドの機密処理 |
...
> **ポイント：** 第2層（APIサーバー）はパブリックに置くが、セキュリティグループで「80/443番ポートのみ受け付ける」と制限する

---

### ③ インターネットゲートウェイ（IGW）

**一言で：** VPCとインターネットをつなぐ「唯一の玄関ドア」

```
重要な特性：
・VPCに1つだけアタッチできる
・IGWが存在しても、サブネットのルートテーブルに
　「0.0.0.0/0 → IGW」の設定がないと通信できない
　→ これが「パブリック/プライベートの分かれ目」
```

**試験でよく出るポイント：**
> 「インターネットに出られないのにIGWにルートが向いていないから」という原因特定の問題が頻出

---

### ④ セキュリティグループ（SG）とネットワークACL

これが**Week 1-2の理解度チェックにある「適用範囲の違い」**です。
...
|---|---|---|
| 適用範囲 | **インスタンス単位**（EC2一台ごと） | **サブネット単位**（フロア全体） |
| デフォルト動作 | 全拒否（許可したものだけ通す） | 全許可（拒否したものだけ止める） |
| ステート | **ステートフル**（戻りの通信は自動許可） | **ステートレス**（行きも帰りも明示的に設定） |
| 主な使い方 | 通常のアクセス制御（ここで9割対応） | サブネット丸ごとブロックしたいとき |

**ステートフル/ステートレスを人間の言葉で：**
```
セキュリティグループ（ステートフル）：
「一度入館を許可した人は、帰りも自動的に通してあげる」

ネットワークACL（ステートレス）：
「入館も退館も、毎回IDカードを確認する」
→ 戻りの通信も明示的に許可しないと通信が失敗する
```
...
graph TD
    ...
    subgraph VPC["VPC 10.0.0.0/16"]
        subgraph AZ_A["AZ-A（東京）"]
            subgraph PubA["パブリック 10.0.1.0/24"]
                ALB["⚖️ ALB\n（ロードバランサー）"]
                EC2a["⚙️ EC2\nAPIサーバー"]
            end
            subgraph PriA["プライベート 10.0.2.0/24"]
                RDSa["🗄️ RDS\n（プライマリ）"]
            end
        end
        subgraph AZ_B["AZ-B（大阪）障害対策"]
            subgraph PubB["パブリック 10.0.3.0/24"]
                EC2b["⚙️ EC2\nAPIサーバー"]
            end
            subgraph PriB["プライベート 10.0.4.0/24"]
                RDSb["🗄️ RDS\n（スタンバイ）"]
            end
        end
    end
...
**なぜAZを2つ使うのか：**
> AZ（アベイラビリティゾーン）＝物理的に離れたデータセンター。1つが災害・障害で落ちても、もう1つが動き続ける。これが**高可用性（HA）設計**の基本。

---

## Week 1-2 理解度チェック：答えられるか確認

**Q1. なぜDBを「プライベートサブネット」に置く必要があるか？**

> ルートテーブルにIGWへの経路が存在しないため、インターネットから物理的に到達できない。設定ミスや攻撃があっても、DBへの経路自体が存在しないため構造的に安全。

**Q2. セキュリティグループとネットワークACLの「適用範囲」の違いは？**

> SGはEC2などのインスタンス単位、NACLはサブネット単位で適用される。さらにSGはステートフル（戻り通信自動許可）、NACLはステートレス（双方向の明示設定が必要）という違いもある。

**Q3. 自宅PCからAWS上のサーバーに接続されるまでの経路は？**

```
自宅PC 
→ インターネット 
→ IGW（VPCの玄関） 
→ パブリックサブネットのルートテーブル確認 
→ ネットワークACLの検問 
→ EC2のセキュリティグループ確認 
→ EC2インスタンスに到達
```
...
| 構成要素 | 役割 | 覚え方 |
|---|---|---|
| VPC | 自分専用の仮想ネットワーク空間 | AWS上のビル |
| サブネット | VPC内の役割別区画 | ビルのフロア |
| IGW | インターネットとの唯一の接続点 | ビルの玄関 |
| ルートテーブル | 通信をどこに転送するかのルール | 館内案内図 |
| セキュリティグループ | インスタンス単位のアクセス制御 | ボディーガード |
| ネットワークACL | サブネット単位のアクセス制御 | フロアの検問所 |
