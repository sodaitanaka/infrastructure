...
flowchart TD
    ...
    B --> C[API: Member（Contact）+ Pet レコード作成\nPet Program = O2O\nStore Code = {StoreCode or AD}]
...
- **Q9**: 登録URLのパス構造はどちらか？
  - 案A: `https://member.nutro.jp/registration/o2o?storeCode=xxx`
  - 案B: `https://member.nutro.jp/registration/pkp?o2o?storeCode=xxx`（PDF記載）
  - フロントエンドの改修範囲に影響するため早期確認が必要

### 【確認】メルマガ・Loyalty Programの扱い

- **Q10**: メルマガ配信・Loyalty Program（将来のアプリ連携）はPKPと共通扱いか？
  - PDFの表では「統一？」と記載あり、方針を確定させたい

### 【確認】アプリ移行時のPW変更問題への方針

- **Q11**: 加藤さんの判断として「PW変更が起こっても先にO2Oを進める」で確定か？
  - これが確定しないとスケジュール（5月中に設計完了）が変わる可能性あり
