...
部分発注の検討用に、全11サブステップの工数・費用・依存関係を一覧化する。

### 9.1 サブステップ別 工数・費用

| # | サブステップ | 作業概要 | Flued<br>（人日） | JIAS<br>（人日） | 開発費<br>（税別） | 前提ステップ |
|:-:|------------|---------|:---:|:---:|---:|:---:|
| 1 | **Step 0-1** | 全システム・データ棚卸し、AS-ISフロー図作成、データ分断ポイント特定 | 4.0 | 1.5 | ¥480,000 | なし |
| 2 | **Step 0-2** | カスタマージャーニー別 TO-BEデータフロー設計（紹介型・売掛型）、Step 1〜3 実装計画確定 | 4.0 | 0.5 | ¥480,000 | 0-1 |
| 3 | **Step 1-1** | 顧客ID名寄せ設計・`int_customer_id_mapping` / `int_unified_customers` dbt実装（メール→電話→氏名の3段階マッチング） | 7.0 | 1.0 | ¥840,000 | 0-2 |
| 4 | **Step 1-2** | 販売形態フラグ設計・実装（紹介型/売掛型の判別ロジック、売掛型の情報取得タイミング設計） | 1.5 | 0.5 | ¥180,000 | 0-2 |
| 5 | **Step 2-1** | 購入履歴マート構築（`int_customer_purchases` + M6 CDP統合顧客 + M7 購買履歴 + M8 RFM/LTV）dbt実装・テスト込み | 18.0 | 1.0 | ¥2,160,000 | 1-1 |
| 6 | **Step 2-2** | BigQuery→HubSpot Reverse ETL構築（Hightouchセットアップ、カスタムプロパティ設計、M6/M8同期モデル実装・テスト） | 9.0 | 1.0 | ¥1,080,000 | 2-1 |
| 7 | **Step 2-3** | Shopify会員データ統合（FiveTranにShopifyコネクタ追加、スマレジ・近江ハグ購買履歴の一元管理） | 1.0 | 0.0 | ¥120,000 | 1-1 |
| 8 | **Step 2-4** | 売掛型顧客 情報取得導線設計（買い替え問い合わせ時の入力フロー・HubSpot登録運用設計） | 2.0 | 0.0 | ¥240,000 | 1-2 |
| 9 | **Step 3-1** | 購入履歴マイページ構築（HubSpot カスタマーポータル設定・購入履歴/サマリー表示・ログインフロー・モバイル対応）【UC-A1】 | 7.0 | 1.0 | ¥840,000 | 2-2 |
| 10 | **Step 3-2** | 買い替えリマインド自動配信（HubSpot Workflow設計・メールテンプレート作成）【UC-A2】 | 2.5 | 1.0 | ¥300,000 | 2-2 |
| 11 | **Step 3-3** | 会員ランク・クーポン管理（RFMセグメント別アクティブリスト設定・クーポン配信Workflow実装）【UC-A3】 | 3.0 | 0.0 | ¥360,000 | 2-2 |
| — | **合計（11サブステップ）** | | **59.0** | **6.5** | **¥6,880,000** | |

> ※ テスト（P2-F）・本番移行（P2-G）の 9.0人日 / ¥1,080,000 は上表に含まない。全ステップ発注時は別途加算。
...
flowchart TD
    ...
    style s01 fill:#6B7280,color:#fff
    style s02 fill:#6B7280,color:#fff
    style s11 fill:#1656A8,color:#fff
    style s12 fill:#1656A8,color:#fff
    style s21 fill:#E8601A,color:#fff
    style s22 fill:#E8601A,color:#fff
    style s23 fill:#1A7A50,color:#fff
    style s24 fill:#1A7A50,color:#fff
    style s31 fill:#7B2FA8,color:#fff
    style s32 fill:#7B2FA8,color:#fff
    style s33 fill:#7B2FA8,color:#fff
```
...
| パターン | 対象サブステップ | Flued工数 | 費用（開発費） | 備考 |
|---------|---------------|:---------:|-------------:|------|
| **① 設計のみ** | 0-1, 0-2 | 8.0人日 | ¥960,000 | 現状整理と全体設計だけ先行発注。内製可否の判断材料に |
| **② 基盤構築まで** | 0-1〜2-2（Step 2-3/2-4除く） | 43.0人日 | ¥5,160,000 | データが揃う状態まで。Step 3はJIAS様で設定も可能 |
| **③ マイページのみ先行** | 0-1, 0-2, 1-1, 2-1, 2-2, 3-1 | 49.0人日 | ¥5,880,000 | UC-A1（マイページ）を最優先で実現するルート |
| **④ フル発注（11サブステップ）** | 全サブステップ | 59.0人日 | ¥6,880,000 | テスト・移行含む場合は +¥1,080,000 |
...
| 1.2 | 2026/04/13 | サブステップ別発注一覧表追加 | 株式会社Flued |
